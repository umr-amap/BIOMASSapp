...
disp_sp <- !is.null(rv$subplot_summary_output)
```
...
# Spatialisation
<font size="4.5">
The coordinates of the plot corners and the relative coordinates of the trees have been checked:
</font>
```

```{r checked_plot, echo=FALSE, fig.align='center', fig.width=6, fig.height=6, message=FALSE, warning=FALSE}
if(disp_sp) {
  if(is.list(rv$checked_plot$plot_design)) {
    for(i in rv$checked_plot$plot_design) {
      print(i)
    }
  } else {
    print(rv$checked_plot$plot_design)
  }
}
```

```{asis, echo = disp_sp & !is.null(rv$file_rast)}
<font size="4.5">
The **background colour** of the plots above corresponds to the **values of the raster provided**.
</font>
```

```{asis, echo = disp_sp & input$check_divide_plot}
<font size="4.5">
The plots were then **divided into subplots**, each measuring `r input$num_grid_size` meters by `r input$num_grid_size` meters.
</font>
```

```{asis, echo = disp_sp}
<font size="4.5">
Therefore, the AGB can be expressed in terms of AGB per hectare (AGBD):
</font>
```

```{r plot_AGBD, echo=FALSE, fig.align='center', fig.width=8, fig.height=7, message=FALSE, warning=FALSE}
if(disp_sp) {
  if(input$rad_several_plots=="several_plots") {
    for(plot_ID in names(rv$subplot_summary_output$plot_design)) {
      print(cowplot::plot_grid(plotlist = rv$subplot_summary_output$plot_design[[plot_ID]][1:length(rv$AGB_res)]))
    }
  } else {
    print(rv$subplot_summary_output$plot_design[1:length(rv$AGB_res)])
  }
}
```
...
```{asis, echo = disp_sp}
<font size="4.5">
Finally, here are all the metrics that were calculated by subplots:
</font>
```

```{r final_results, echo=FALSE, fig.align='center', fig.width=8, fig.height=7, message=FALSE, warning=FALSE, results='asis'}
if(disp_sp) {
  tab <- rv$FOS_subplot
  tab <- tab |> 
    kable(format = "html", caption = "FOS like results at subplot level") |> 
    kable_styling(
      bootstrap_options = c("striped", "hover", "condensed", "responsive"),
      full_width = TRUE,
      position = "center"
    )  |> 
    row_spec(0, background = "#156620", color = "white", bold = TRUE)
  
  div(style = "overflow-x:auto; white-space:nowrap; max-width:100%",
      HTML(tab, format = "html", digits = 3, align="c")
  )
}
```
