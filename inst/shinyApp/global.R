...
indiv_H_pred <- function(inv, rad_height, H, AGB_res, chkgrp_HEIGHT, sel_HDmodel_by, hd_data, hd_model, D, region, coord){
  ...
  if(!is.null(rad_height) && rad_height == "h_each_tree") {
    ...
    inv_h_pred$AGB <- as.vector(AGB_res[["height"]]$AGB_pred)
  }
  if ("HDloc" %in% chkgrp_HEIGHT) {
    ...
    inv_h_pred$AGB_local_model <- as.vector(AGB_res[["local HD model"]]$AGB_pred)
  }
  if ("feld" %in% chkgrp_HEIGHT) {
    ...
    inv_h_pred$AGB_Feldpausch <- as.vector(AGB_res[["Feldpausch"]]$AGB_pred)
  }
  if ("chave" %in% chkgrp_HEIGHT) {
    ...
    inv_h_pred$AGB_Chave <- as.vector(AGB_res[["Chave"]]$AGB_pred)
  }
  return(inv_h_pred)
}
