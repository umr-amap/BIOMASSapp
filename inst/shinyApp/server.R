function(input, output, session) {
  ...
  # Set the plot column that contains plotID (in inv)
  observeEvent(
    list(input$rad_several_plots, input$sel_PLOT), ignoreInit = TRUE, ignoreNULL = TRUE, {
      req(rv$inv)
      print("Set the plot column that contains plotID")
      inv_plot <- rv$inv
    ...
    if ("HDloc" %in% input$chkgrp_HEIGHT) {
      ...
      if (input$rad_height == "h_some_tree") { # if height of some trees in the same dataset
        ...
        if(input$sel_HDmodel_by != "<unselected>"){ # if one model by plot or region or whatever
          ...
          rv$hd_data <- new_hd_data[!model_for %in% removedPlot]
  ...
  observeEvent({
    list(input$chkgrp_HEIGHT, input$rad_HDMOD, input$btn_DATASET_LOADED)
    ...
    if ("chave" %in% input$chkgrp_HEIGHT) {
      ...
      # creating model prediction lines for each plots/regions/...
      if (!is.list(rv$E)) { # E is a list if an error was caught
  ...
  observeEvent(input$btn_AGB_DONE, {
    ...
    output$out_plot_AGB <- renderPlot({
      plot_list(rv$AGB_sum, color, AGBmod = AGBmod)
    })
  ...
  output$dwl_tree_file <- downloadHandler(
    ...
    content = function(file) {
      ...
      showModal(modalDialog("Loading... (can take a while for Chave's height predictions if you have a poor connexion)", footer=NULL))
      on.exit(removeModal())

      if(is.null(rv$inv_h_pred)) { # rv$inv_h_pred can also be created in plot level results (see below)
      ...
      n_plot_col <- match("plot", names(rv$inv_h_pred))[length(match("plot", names(rv$inv_h_pred)))] # column number of the plot columnn (use of [length(...)] in case user has already a "plot" column)
      write.csv(as.data.frame(rv$inv_h_pred[,-n_plot_col]), file, row.names = FALSE)
    },
  ...
  output$dwl_plot_file <- downloadHandler(
    ...
    content = function(file) {
      ...
      showModal(modalDialog("Loading... (may take a while for Chave's height predictions if you have a poor connexion)", footer=NULL))
      on.exit(removeModal())

      ### results will contain for each plot (if the column exists):
      # Plot_ID, Ndens, Lat_cnt, Lon_cnt, MinDBH, BA, Wood density(mean), H_Lorey_..., H_max_..., AGB_..., AGB_..._Cred_2.5, AGB_..._Cred_97.5
      ...
      list_fun <- list("Ndens"=min, "D" = min, "BA"=sum, "WD"=mean,
