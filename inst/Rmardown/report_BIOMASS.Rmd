...
if (RUN) {
  ...
  if (all((sapply(coord[, 1:2], class) == "numeric"))) {
    ...
    if (nrow(coord) != 0) {
      leaflet(coord) %>%
        addTiles() %>%
        addMarkers(lng = ~longitude, lat = ~latitude, popup = ~plot)
    }
  }
}
```
...
```{r feldpausch region}
if (RUN) {
  feldRegion <- c(Africa = "Africa", CAfrica = "Central Africa", EAfrica = "Eastern Africa", WAfrica = "Western Africa", SAmerica = "South America", BrazilianShield = "Brazilian Shield", ECAmazonia = "Eastern-central Amazonia", GuianaShield = "Guiana Shield", WAmazonia = "Western Amazonia", SEAsia = "Southeast Asia", NAustralia = "Northern Australia", Pantropical = "Pantropical")
