...
if (RUN) {
  ...
  coord <- data.table(
    ...
    plot = if ("plot" %in% names(data)) data$plot else rep(input$num_LONG, "plot")
  )
...
We decided to use a chave 2004 HD model. The range of E is: 
```{r}
round(range(computeE(cbind(coord$longitude, coord$latitude))), digits = 3)
```
...
`r if (!RUN) {"<!--"}`
The error for the wood density is 0.
`r if (!RUN) {"-->"}`
...
```{r plot biomass, fig.align='center', fig.width=fig_width, fig.height=fig_height, warning=FALSE}
